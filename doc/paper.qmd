...
`EM1` is a measure that assesses how much insiders (e.g., company executives) smooth or stabilize reported earnings by manipulating accruals. The underlying idea is that firms may attempt to reduce fluctuations in their reported operating income to present a more stable financial performance to investors. This measure indicates the degree to which insiders exercise accounting discretion to smooth reported earnings, with lower values of EM1 suggesting more aggressive earnings smoothing.

To calculate this measure:
1.	Accruals are computed using changes in various components of a firm’s balance sheet and income statement, such as total current assets, cash, current liabilities, short-term debt, taxes payable, and depreciation.
$$
...
\text{Dep}_{it} & = \text{Depreciation and amortization expense}
\end{aligned}
$$

2.	The operating cash flow is indirectly derived from operating income minus accruals
$$
\text{CFO}_{it} = \text{Operating Income}_{it} - \text{Accruals}_{it}
$$

3.	EM1 is then calculated as the country’s median ratio of the firm-level standard deviations of operating income and operating cash flow. Both these standard deviations are scaled by lagged total assets to control for differences in firm size.
$$
\text{EM1} = \text{Median}\left(\frac{\sigma(\text{Operating Income}_{it})}{\sigma(\text{Operating Cash Flow}_{it})}\right) \quad \text{scaled by lagged total assets}
$$
